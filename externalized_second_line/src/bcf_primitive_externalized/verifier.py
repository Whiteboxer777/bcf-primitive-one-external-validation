...
from bcf_primitive_verifier.contract import VERDICT_SCHEMA, bundle_contract
from bcf_primitive_verifier.governance import evaluate_authority, verify_signature
from bcf_primitive_verifier.kernel_support import compare_profiles, evaluate_profile, extract_decisive_core, is_satisfiable, minimize_profile, normalize, parse_profile, Profile
from .common import digest_data, load_json, sha256_file, PRIMITIVE_IDENTITY
...
from bcf_primitive_verifier.admission_normal_form import compile_admission_normal_form
from bcf_primitive_verifier.compiled_backend import compile_compiled_backend
from bcf_primitive_verifier.witness_certificate import attach_witness_certificate, WITNESS_CERTIFICATE_SCHEMA
...
_PRECEDENCE = ['required_files', 'top_level', 'contract', 'manifest', 'compiler_products', 'certificates', 'governance', 'verdict_schema', 'verification_schema', 'refusal_schema', 'permit_schema', 'compiled_backend_schema', 'witness_certificate_schema', 'claim_and_tcb', 'claim_matrix', 'witness_replay']
...
def _verification_result(*, bundle_digest: str | None, overall_ok: bool, checks: dict[str, bool], details: dict[str, Any], refusal_reasons: list[str]) -> dict[str, Any]:
    if overall_ok:
        return attach_witness_certificate({'primitive_identity': PRIMITIVE_IDENTITY,'verification_result': 'VERIFIED','overall_ok': True,'bundle_digest': bundle_digest,'checks': checks,'details': details,'replay_binding': {'primitive_identity': PRIMITIVE_IDENTITY, 'bundle_digest': bundle_digest, 'request_digest': None},})
    primary_name = next(name for name in _PRECEDENCE if not checks.get(name, True))
    primary_code = REFUSAL_CODE_MAP[primary_name]
    refusal = make_refusal(code=primary_code, layer='bundle', bundle_digest=bundle_digest, verification_result='NOT_VERIFIED', refusal_reasons=refusal_reasons, details=details)
    refusal.update({'overall_ok': False, 'checks': checks})
    return attach_witness_certificate(refusal)

def verify_bundle_externalized(bundle_dir: str | Path) -> dict[str, Any]:
    bundle = Path(bundle_dir)
    required = _verify_required_files(bundle)
    if not required['ok']:
        return _verification_result(bundle_digest=None, overall_ok=False, checks={'required_files': False}, details={'required_files': required}, refusal_reasons=['REFUSE_REQUIRED_FILES_MISSING'])
    ...
    checks = {'required_files': required['ok'],'top_level': top_level['ok'],'contract': contract['ok'],'manifest': manifest['ok'],'compiler_products': compiler['ok'],'certificates': certs['ok'],'governance': governance['ok'],'verdict_schema': verdict_schema['ok'],'verification_schema': verification_schema['ok'],'refusal_schema': refusal_schema['ok'],'permit_schema': permit_schema['ok'],'compiled_backend_schema': compiled_backend_schema['ok'],'witness_certificate_schema': witness_certificate_schema['ok'],'claim_and_tcb': claim_tcb['ok'],'claim_matrix': claim_matrix['ok'],'witness_replay': witnesses['ok']}
    details = {'contract_version': bundle_contract.version,'required_files': required,'top_level': top_level,'contract': contract,'manifest': manifest,'compiler_products': compiler,'certificates': certs,'governance': governance,'verdict_schema': verdict_schema,'refusal_schema': refusal_schema,'permit_schema': permit_schema,'compiled_backend_schema': compiled_backend_schema,'witness_certificate_schema': witness_certificate_schema,'claim_and_tcb': claim_tcb,'claim_matrix': claim_matrix,'witness_replay': witnesses}
